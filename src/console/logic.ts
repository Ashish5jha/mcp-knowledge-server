import { Env, Manifest, PROFILES, Profile, RegistryConfig, SearchIndex, DocumentMeta } from "../types";
...
function diffDocs(current: DocumentMeta, next: DocumentMeta): CompareDocChange["changes"] {
  ...
  return fields
    .filter((field) => JSON.stringify(currentDoc[field]) !== JSON.stringify(nextDoc[field]))
    .map((field) => ({
      field,
...
async function getProfileSource(env: Env, profile: Profile): Promise<ProfileSource> {
...
async function fetchLiveManifest(env: Env, profile: Profile): Promise<GitHubManifestState> {
...
async function getCloudflareIndex(env: Env, profile: Profile): Promise<CloudflareIndexState> {
  ...
  return {
    ...
    documentCount: Array.isArray(index.documents) ? index.documents.length : 0,
    recentDocs: sortByRecentUpdate(index.documents).slice(0, 5),
  };
}

function determineFreshness(index: CloudflareIndexState, manifest: GitHubManifestState): FreshnessState {
  if (!index.exists) {
    return { state: "missing", reason: "No Cloudflare index found for this profile" };
  }

  if (!manifest.reachable) {
    return {
      state: "error",
      reason: manifest.error ? `Unable to fetch GitHub manifest: ${manifest.error}` : "Unable to fetch GitHub manifest",
    };
  }

  if (!index.version || !manifest.version) {
    return { state: "error", reason: "Missing version information on index or manifest" };
  }
  ...
  return { state: "fresh", reason: "Cloudflare index matches the current GitHub manifest" };
}
...
export async function getConsoleStatus(env: Env, profile: Profile): Promise<ConsoleStatus> {
  const registry = await getProfileSource(env, profile);
  const [index, manifest] = await Promise.all([getCloudflareIndex(env, profile), fetchLiveManifest(env, profile)]);
...
export async function compareProfile(env: Env, profile: Profile): Promise<CompareResult> {
  const index = await getCache<SearchIndex>(env, `index:${profile}`);
  ...
  const addedDocs = manifest.documents.filter((doc) => !indexDocs.has(doc.id));
  const removedDocs = index.documents.filter((doc) => !manifestDocs.has(doc.id));
  ...
  return {
    profile,
    summary: {
      state: addedDocs.length === 0 && removedDocs.length === 0 && changedDocs.length === 0 ? "in_sync" : "out_of_sync",
...
export function searchProfile(index: SearchIndex, query: string, limit: number): SearchHit[] {
  ...
  for (const doc of index.documents) {
    ...
    const tagTerms = new Set([...doc.tags.flatMap(tokenize), ...doc.keywords.flatMap(tokenize)]);
  ...
  return Object.entries(scores)
    ...
    .map(([id, score]) => {
      ...
      const confidence: SearchHit["confidence"] = ratio >= 0.8 ? "exact" : ratio >= 0.4 ? "partial" : "related-only";
...
export async function getSearchResults(env: Env, profile: Profile, query: string, limit: number): Promise<{
...
export async function getDocument(env: Env, profile: Profile, id: string): Promise<ConsoleDocument> {
  const index = await getCache<SearchIndex>(env, `index:${profile}`);
  if (!index) {
...
export async function reindexProfile(env: Env, profile: Profile): Promise<SearchIndex> {
